...
{
	name = "SourceComms",
	author = "Alex, SourceBans++ Dev Team, Kyle",
...
public OnPluginStart()
{
	...
	{
		SetFailState("Database failure: could not find database config: %s", "csgo");
		return;
	}
	DB_Connect();
...
public void ServerInfoCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	{
		LogError("Server Select Query Failed: %s", error);
		return;
	}
	
	if(SQL_FetchRow(hndl))
		ServerID = SQL_FetchInt(hndl, 0);
}
...
public Query_VerifyBlock(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
	...
	if (SQL_GetRowCount(hndl) > 0)
	{
		while (SQL_FetchRow(hndl))
		{
			...
			if (aid <= 0 && ConsoleImmunity > immunity)
...
stock bool:DB_Connect()
{
	...
	if (g_DatabaseState != DatabaseState_Connecting)
	{
		...
		SQL_TConnect(GotDatabase, "csgo", g_iConnectLock);
	}
...
stock bool:TempUnBlock(Handle:data)
{
	...
	new bool:AdmImCheck = ((type == TYPE_UNMUTE && AdmImmunity >= g_iMuteLevel[target]) || (type == TYPE_UNGAG && AdmImmunity >= g_iGagLevel[target]) || (type == TYPE_UNSILENCE && AdmImmunity >= g_iMuteLevel[target] && AdmImmunity >= g_iGagLevel[target]));
	...
	new bool:bHasPermission = (!admin && StrEqual(adminAuth, "STEAM_ID_SERVER")) || AdmImCheck;
...
stock SavePunishment(admin = 0, target, type, length = -1, const String:reason[] = "")
{
	...
	if (DB_Connect())
	{
		...
		LogError("SavePunishment -> !DB_Connect()");
}
