...
void SendChatToAll(int client, const char[] message)
{
	for (int x = 1; x <= MaxClients; ++x)
	{
		...
		if (CheckCommandAccess(x, "sm_say", ADMFLAG_CHAT))
		{
			PrintToChat(x, "***\x0C%N\x01***\x07   %s", client, message);
		}
		else
		{
			PrintToChat(x, "***\x0C管理员\x01***\x07   %s", message);
		}
	}

	EmitSoundToAll("buttons/button18.wav");
	LogMessage("\"%L\" sm_say %s", client, message);
}
