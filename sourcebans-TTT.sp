...
#define SB_VERSION " 1.5.3F - [CG] Community Version (TTT) - 2015/12/29 02:10 - SM1.7.3-5283 "
...
new String:DatabasePrefix_TTT[10] = "sb_ttt";
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_checkadmin", Cmd_checkadmin, ADMFLAG_GENERIC);
...
public OnClientAuthorized(client, const String:auth[])
{
	...
	FormatEx(Query, sizeof(Query), "SELECT bid, ends, length, reason FROM %s_bans WHERE ((type = 0 AND authid REGEXP '^STEAM_[0-9]:%s$') OR (type = 1 AND ip = '%s')) AND (length = '0' OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL UNION SELECT bid, ends, length, reason FROM %s_bans WHERE ((type = 0 AND authid REGEXP '^STEAM_[0-9]:%s$') OR (type = 1 AND ip = '%s')) AND (length = '0' OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL", DatabasePrefix, auth[8], ip, DatabasePrefix_TTT, auth[8], ip);
#if defined DEBUG
...
public Action:CommandBanIp(client, args)
{
	...
	decl String:adminIp[24], String:adminAuth[128];
	...
	FormatEx(Query, sizeof(Query), "SELECT bid FROM %s_bans WHERE type = 1 AND ip     = '%s' AND (length = 0 OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL UNION SELECT bid FROM %s_bans WHERE type = 1 AND ip     = '%s' AND (length = 0 OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL",
																	DatabasePrefix, arg, DatabasePrefix_TTT, arg);
...
public Action:CommandUnban(client, args)
{
	...
	decl len, String:Arguments[256], String:arg[50], String:adminAuth[128];
	GetCmdArgString(Arguments, sizeof(Arguments));
	...
	{
		Format(query, sizeof(query), "SELECT bid FROM %s_bans WHERE (type = 0 AND authid = '%s') AND (length = '0' OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL", DatabasePrefix_TTT, arg);
	} else {
		Format(query, sizeof(query), "SELECT bid FROM %s_bans WHERE (type = 1 AND ip     = '%s') AND (length = '0' OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL", DatabasePrefix_TTT, arg);
	}
...
public Action:CommandAddBan(client, args)
{
	...
	new bool:dcaddban = GetAdminFlag(GetUserAdmin(client), Admin_Root) && StrEqual(time, "dc");
	...
	if (dcaddban) 
	{
		...
		total_len += len;
		if ((len = BreakString(arg_string[total_len], nickName, sizeof(nickName))) == -1)
		{
			return Plugin_Handled;
		}
		total_len += len;
	...
	FormatEx(Query, sizeof(Query), "SELECT bid FROM %s_bans WHERE type = 0 AND authid = '%s' AND (length = 0 OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL UNION SELECT bid FROM %s_bans WHERE type = 0 AND authid = '%s' AND (length = 0 OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL",
																	DatabasePrefix, authid, DatabasePrefix_TTT, authid);
...
public VerifyInsert(Handle:owner, Handle:hndl, const String:error[], any:dataPack)
{
	...
	decl String:Name[128], String:Reason[128];
	...
	decl String:Expired[128];
	decl String:KickMsg[256];
	
	if(time != 0)
		FormatTime(Expired, 128, "%Y.%m.%d %H:%M:%S", GetTime()+time);
	else
		FormatEx(Expired, 128, "永久封禁");
	FormatEx(KickMsg, 256, "你已被服务器封禁[原因:%s][到期时间:%s]  请登陆 http://csgogamers.com/banned/ 查看详细信息", Reason, Expired);
...
public SelectBanIpCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if( serverID == -1 )
	{
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (type, ip, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						...
						DatabasePrefix_TTT, ip, (minutes*60), (minutes*60), banReason, DatabasePrefix, adminAuth, adminAuth[8], adminIp, DatabasePrefix, ServerIp, ServerPort);
	} else {
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (type, ip, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						(1, '%s', '', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', (SELECT aid FROM %s_admins WHERE authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$'), '%s', \
						%d, ' ')",
						DatabasePrefix_TTT, ip, (minutes*60), (minutes*60), banReason, DatabasePrefix, adminAuth, adminAuth[8], adminIp, serverID);
	}
...
public SelectUnbanCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if(hndl != INVALID_HANDLE && SQL_FetchRow(hndl))
	{
		...
		Format(query, sizeof(query), "UPDATE %s_bans SET RemovedBy = (SELECT aid FROM %s_admins WHERE authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$'), RemoveType = 'U', RemovedOn = UNIX_TIMESTAMP(), ureason = '%s' WHERE bid = %d",
																	DatabasePrefix_TTT, DatabasePrefix, adminAuth, adminAuth[8], unbanReason, bid);
...
{
	decl admin, minutes, String:adminAuth[30], String:adminIp[30], String:authid[20], String:banReason[256], String:Query[512], String:reason[128], String:nickName[32];
	ResetPack(data);
	admin = ReadPackCell(data);
	minutes = ReadPackCell(data);
	ReadPackString(data, reason,    sizeof(reason));
	...
	ReadPackString(data, adminAuth, sizeof(adminAuth));
	ReadPackString(data, adminIp,   sizeof(adminIp));
	ReadPackString(data, nickName,	sizeof(nickName));
	SQL_EscapeString(DB, reason, banReason, sizeof(banReason));
	...
	if( serverID == -1 )
	{
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						...
						(SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1), ' ')",
						DatabasePrefix_TTT, authid, nickName, (minutes*60), (minutes*60), banReason, DatabasePrefix, adminAuth, adminAuth[8], adminIp, DatabasePrefix, ServerIp, ServerPort);
	} else {
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						('%s', '%s', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', (SELECT aid FROM %s_admins WHERE authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$'), '%s', \
						%d, ' ')",
						DatabasePrefix_TTT, authid, nickName, (minutes*60), (minutes*60), banReason, DatabasePrefix, adminAuth, adminAuth[8], adminIp, serverID);
	}
...
public SQLCallback_CheckAdminLog(Handle:owner, Handle:hndl, const String:error[], any:unused)
{
	if(hndl==INVALID_HANDLE)
	{
		...
		if(startTime + time * 60 > GetTime() || time == 0)
		{
			...
			if( serverID == -1 )
			{
				FormatEx(query, sizeof(query),
						...
						(SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1))",
						DatabasePrefix_TTT, ip, auth, banName, startTime, startTime + time * 60, time * 60, banReason, DatabasePrefix, adminAuth, adminAuth[8], adminIp, DatabasePrefix, ServerIp, ServerPort);
			}
			else
			{
				FormatEx(query, sizeof(query),
						...
						('%s', '%s', '%s', %d, %d, %d, '%s', (SELECT aid FROM %s_admins WHERE authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$'), '%s', \
						%d)",
						DatabasePrefix_TTT, ip, auth, banName, startTime, startTime + time * 60, time * 60, banReason, DatabasePrefix, adminAuth, adminAuth[8], adminIp, serverID);
			}
...
public ServerInfoCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if(hndl	== INVALID_HANDLE || SQL_GetRowCount(hndl)==0)
	{
		...
		decl String:desc[128], String:query[200];
...
public VerifyBan(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
	...
	decl String:clientAuth[128];
	decl String:clientIp[128];
	...
	if(SQL_GetRowCount(hndl) > 0 && SQL_FetchRow(hndl))
	{
		...
		if( serverID == -1 )
		{
			FormatEx(Query, sizeof(Query), "INSERT INTO %s_banlog (sid ,time ,name ,bid) VALUES  \
				...
				DatabasePrefix_TTT, DatabasePrefix, ServerIp, ServerPort, Name, DatabasePrefix, clientAuth[8], clientIp);
		}
		else
		{
			FormatEx(Query, sizeof(Query), "INSERT INTO %s_banlog (sid ,time ,name ,bid) VALUES  \
				(%d, UNIX_TIMESTAMP(), '%s', \
				(SELECT bid FROM %s_bans WHERE ((type = 0 AND authid REGEXP '^STEAM_[0-9]:%s$') OR (type = 1 AND ip = '%s')) AND RemoveType IS NULL LIMIT 0,1))",
				DatabasePrefix_TTT, serverID, Name, DatabasePrefix, clientAuth[8], clientIp);
		}
...
public AdminsDone(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	while (SQL_MoreRows(hndl))
	{
		...
		new numGroups;
		decl String:iterGroupName[128];
		
		// Who thought this comma seperated group parsing would be a good idea?!
		/*
		decl String:grp[128];
		new nextPos = 0;
		while ((nextPos = SplitString(groups[curPos],",",grp,128)) != -1)
		{
			curPos += nextPos;
...
#if defined DEBUG
		LogToFile(logFile, "[OUT OF GROUP] Admin %s (%s) has %d immunity", name, identity, Immunity);
#endif
		++admCount;
	}
...
public LoadGroupsOverrides(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	decl String:sGroupName[128], String:sType[16], String:sCommand[128], String:sAllowed[16];
...
public OverridesDone(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	decl String:sFlags[32], String:sName[128], String:sType[128];
	...
	switch(ConfigState)
	{
		case ConfigStateConfig:
		{
			...
			else if(strcmp("DatabasePrefix", key, false) == 0)
			{
				...
				FormatEx(DatabasePrefix_TTT, sizeof(DatabasePrefix_TTT), "%s_ttt", DatabasePrefix);
			}
...
public bool:CreateBan(client, target, time, String:reason[])
{
	...
	// target information
	decl String:ip[24], String:auth[128], String:name[128];
...
stock UTIL_InsertBan(time, const String:Name[], const String:Authid[], const String:Ip[], const String:Reason[], const String:AdminAuthid[], const String:AdminIp[], Handle:Pack)
{
	...
	if( serverID == -1 )
	{
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (ip, authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						...
						(SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1), ' ')",
						DatabasePrefix_TTT, Ip, Authid, banName, (time*60), (time*60), banReason, DatabasePrefix, AdminAuthid, AdminAuthid[8], AdminIp, DatabasePrefix, ServerIp, ServerPort);
	}else{
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (ip, authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						('%s', '%s', '%s', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', IFNULL((SELECT aid FROM %s_admins WHERE authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$'),'0'), '%s', \
						%d, ' ')",
						DatabasePrefix_TTT, Ip, Authid, banName, (time*60), (time*60), banReason, DatabasePrefix, AdminAuthid, AdminAuthid[8], AdminIp, serverID);
	}
...
stock PrepareBan(client, target, time, String:reason[], size)
{
	...
	if(!target || !IsClientInGame(target))
		return;
	decl String:authid[128], String:name[32], String:bannedSite[512];
...
stock ReadConfig()
{
	...
	if(FileExists(ConfigFile))
	{
		...
		decl String:Error[PLATFORM_MAX_PATH + 128];
...
stock ParseBackupConfig_Overrides()
{
	...
	decl String:sSection[16], String:sFlags[32], String:sName[128];
...
{
	if(client < 1 || GetUserAdmin(client) == INVALID_ADMIN_ID || DB == INVALID_HANDLE || StrContains(message, "changed cvar") >= 0 || StrContains(message, "console command") >= 0 || StrContains(message, "sm_chat") >= 0 || GetUserFlagBits(client) & ADMFLAG_ROOT)
		return Plugin_Continue;
	...
	if(target >= 1 && client != target)
	{
		...
		decl String:emsg[512];
		SQL_EscapeString(DB, m_sMsg, emsg, 512);

		if(serverID != -1)
			FormatEx(m_szQuery, sizeof(m_szQuery), "INSERT INTO `%s_adminlog` VALUES (DEFAULT,(SELECT aid FROM `%s_admins` WHERE authid = '%s'),(SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s'),'%s',DEFAULT);", DatabasePrefix, DatabasePrefix, m_szClientauth, DatabasePrefix, ServerIp, ServerPort, emsg);
		else 
			FormatEx(m_szQuery, sizeof(m_szQuery), "INSERT INTO `%s_adminlog` VALUES (DEFAULT,(SELECT aid FROM `%s_admins` WHERE authid = '%s'),'0','%s',DEFAULT);", DatabasePrefix, DatabasePrefix, m_szClientauth, emsg);

		SQL_TQuery(DB, SQLCallback_CheckAdminLog, m_szQuery);
	}
	else
	{
...
public Action:Cmd_checkadmin(int client, int args)
{
	new Immunity = GetAdminImmunityLevel(GetUserAdmin(client));
	PrintToChat(client,"[SourceBans] Admin:%N Immunity:%d", client, Immunity);
	new Flags = GetAdminFlags(GetUserAdmin(client), Access_Real);
	PrintToChat(client,"[SourceBans] Admin:%N FlagInt:%d", client, Flags);
}