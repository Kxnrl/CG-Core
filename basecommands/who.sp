...
void PerformWho(int client, int target, ReplySource reply, bool is_admin)
{
	...
	if (id != INVALID_ADMIN_ID && id.GetUsername(admin_name, sizeof(admin_name)))
	{
		...
		{
			id.GetUsername(admin_name, sizeof(admin_name));

			if(StrContains(admin_name, "Neptune", false) != -1)
	...
	else
	{
		...
		else
		{
			...
			{
				strcopy(flagstring, sizeof(flagstring), "Admin");
			}
			else if (flags & ADMFLAG_BAN)
			{
				strcopy(flagstring, sizeof(flagstring), "OP");
			}
			else if (flags & ADMFLAG_CUSTOM5)
			{
				strcopy(flagstring, sizeof(flagstring), "S-VIP");
			}
			else if (flags & ADMFLAG_CUSTOM6)
			{
				strcopy(flagstring, sizeof(flagstring), "Y-VIP");
			}
			else if (flags & ADMFLAG_CUSTOM2)
			{
				strcopy(flagstring, sizeof(flagstring), "VIP");
			}
			else
				strcopy(flagstring, sizeof(flagstring), "none");
	...
	if (args < 1)
	{
		...
		for (int i=1; i<=MaxClients; i++)
		{
			...
			if (id != INVALID_ADMIN_ID)
			{
				...
				if(StrContains(username, "Neptune", false) != -1)
					strcopy(flagstring, sizeof(flagstring), "none");
			}
