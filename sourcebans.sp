...
public Plugin myinfo =
{
    ...
    version     = "2.1+dev12 [Base on SB-1.5.3F]",
...
public void OnClientAuthorized(int client, const char[] auth)
{
    ...
    char reason[256];
    if(GetTrieString(g_aBanList, auth, reason, 256))
    {
...
public void VerifyInsert(Handle owner, Handle hndl, const char[] error, any dataPack)
{
    ...
    FormatEx(KickMsg, 256, "您已被服务器封锁,禁止进入游戏!\n类型: %s\n原因: %s\n到期: %s\n访问https://csgogamers.com/banned/查看详细信息", bantype, Reason, Expired);
    
    char clientAuth[32];
    GetClientAuthId(client, AuthId_Steam2, clientAuth, 32, true);
    SetTrieString(g_aBanList, clientAuth, KickMsg, true);
    Handle pack;
    CreateDataTimer(600.0, Timer_RemoveTrie, pack);
    WritePackString(pack, clientAuth);
    // Kick player
    if(GetClientUserId(client) == UserId)
        KickClient(client, KickMsg);
}

public Action Timer_RemoveTrie(Handle timer, Handle pack)
{
    ResetPack(pack);
    char auth[32];
    ReadPackString(pack, auth, 32);
    RemoveFromTrie(g_aBanList, auth);
    return Plugin_Stop;
}
        ...
        while(SQL_FetchRow(hndl))
        {
            ...
            if(StrEqual(bType, "全服封禁") || StrEqual(bType, bantype) || (StrEqual(bType, "单服封禁") && sid == g_iServerId))
            {
                ...
                if(Length != 0)
                    ...
                    FormatEx(Expired, 128, "永久封禁");
                FormatEx(KickMsg, 256, "您已被服务器封锁,禁止进入游戏!\n类型: %s\n原因: %s\n到期: %s\n访问https://csgogamers.com/banned/查看详细信息", bType, Reason, Expired);
                SetTrieString(g_aBanList, clientAuth, KickMsg, true);
                Handle pack;
                CreateDataTimer(600.0, Timer_RemoveTrie, pack);
                WritePackString(pack, clientAuth);
