...
#define SB_VERSION " 1.5.3F - [CG] Community Version - 2015/12/29 02:09 - SM1.7.3-5283 "
...
public OnClientAuthorized(client, const String:auth[])
{
	...
	FormatEx(Query, sizeof(Query), "SELECT bid, ends, length, reason FROM %s_bans WHERE ((type = 0 AND authid REGEXP '^STEAM_[0-9]:%s$') OR (type = 1 AND ip = '%s')) AND (length = '0' OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL", DatabasePrefix, auth[8], ip);
#if defined DEBUG
...
public SelectAddbanCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if( serverID == -1 )
	{
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						...
						(SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1), ' ')",
						DatabasePrefix, authid, nickName, (minutes*60), (minutes*60), banReason, DatabasePrefix, adminAuth, adminAuth[8], adminIp, DatabasePrefix, ServerIp, ServerPort);
	} else {
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						('%s', '%s', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, %d, '%s', (SELECT aid FROM %s_admins WHERE authid = '%s' OR authid REGEXP '^STEAM_[0-9]:%s$'), '%s', \
						%d, ' ')",
						DatabasePrefix, authid, nickName, (minutes*60), (minutes*60), banReason, DatabasePrefix, adminAuth, adminAuth[8], adminIp, serverID);
	}
...
#if defined DEBUG
				LogToFile(logFile, "Admin %s (%s) has %d immunity(old)", name, identity, GetAdminImmunityLevel(curAdm));
#endif
